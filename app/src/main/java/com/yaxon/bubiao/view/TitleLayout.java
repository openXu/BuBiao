...
import com.yaxon.bubiao.utils.FClickUtil;
...
public class TitleLayout extends RelativeLayout {
    ...
    private float menuTextSize;          //文字大小
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        LayoutInflater.from(context).inflate(R.layout.view_custom_title_layout, this, true);
...
//        view_left = findViewById(R.id.view_left);
        ...
        if (attrs != null) {
            ...
            titleTextSize = ta.getDimension(R.styleable.TitleView_titleTextSize, 1);
            menuTextSize = ta.getDimension(R.styleable.TitleView_menuTextSize, 1);
...
/*
            ...
            FLog.i(TAG, "backgroundDrawable="+backgroundDrawable);
            int defTitleColor = getResources().getColor(R.color.colorPrimary);
            ...
            if(null == backgroundDrawable){
                ...
                FLog.i(TAG, "设置title背景色"+background);
                setBackgroundColor(background);
            }else{
                background = ta.getResourceId(R.styleable.TitleView_android_background, -1);
                FLog.i(TAG, "设置title背景资源图片"+background);
                setBackgroundResource(background);
            }
            FLog.i(TAG, "background = "+background);*/
//
//        FLog.i(TAG, "textLeft = "+textLeft);
//        FLog.i(TAG, "textcenter = "+textcenter);
//        FLog.i(TAG, "textRight = "+textRight);
//        FLog.i(TAG, "iconBack = " + iconBack);
//        FLog.i(TAG, "iconLeft = "+iconLeft);
//        FLog.i(TAG, "iconCenterRow = "+iconCenterRow);
//        FLog.i(TAG, "iconRight = "+iconRight);
//        FLog.i(TAG, "iconRight = "+iconRight);
//        FLog.i(TAG, "textSize = "+textSize);
//        FLog.i(TAG, "textColor = "+textColor);
//        FLog.i(TAG, "textIconSpace = "+textIconSpace);
//        FLog.i(TAG, "leftSpace = " + leftSpace);
//        FLog.i(TAG, "rightSpace = "+rightSpace);
//        FLog.i(TAG, "centerRowSpace = "+centerRowSpace);
    ...
    private void setParams() {
        ...
        tv_left_text.setTextSize(TypedValue.COMPLEX_UNIT_PX, menuTextSize);
        tv_center_text.setTextColor(textColor);
        tv_center_text.setTextSize(TypedValue.COMPLEX_UNIT_PX, titleTextSize);
        tv_right_text.setTextColor(textColor);
        tv_right_text.setTextSize(TypedValue.COMPLEX_UNIT_PX, menuTextSize);
...
//            FLog.i(TAG, "版本："+Build.VERSION.SDK_INT+" >= "+Build.VERSION_CODES.KITKAT+"，需要显示状态栏占位");
...
//        FLog.i("宽度："+MeasureSpec.getSize(widthMeasureSpec));
//        FLog.i("搞度："+MeasureSpec.getSize(heightMeasureSpec));
    }
...
//                FLog.d(TAG, "左侧文字可见"+ tv_left_text.getMeasuredWidth());
//                FLog.d(TAG, "左侧文字可见"+ FontUtil.getFontlength(textPaint, textLeft));
                leftUsed += tv_left_text.getMeasuredWidth();

                tv_left_text.setOnClickListener(v -> {
                    if (FClickUtil.isNotFastClick() && listener != null) {
...
//                FLog.d(TAG, "右侧文字可见"+tv_left_text.getMeasuredWidth());
//                FLog.d(TAG, "右侧文字可见"+FontUtil.getFontlength(textPaint, textRight));
...
//                FLog.i(TAG, "气泡的宽高："+widthPop+" * "+heightPop);
            ...
            if (null != rightIconArr && rightIconArr.length > 0) {
                ...
                for (int id : rightIconArr) {
                    ...
                    content_params.leftMargin = FConversUtils.dip2px(getContext(), 10);
                    ImageView iv = new ImageView(getContext());
                    iv.setImageResource(id);
                    FLog.e("设置图片==" + id);
                    ll_right_icon_content.addView(iv, content_params);
                    iv.setOnClickListener(v -> {
                        if (FClickUtil.isNotFastClick()&&contentIconListener != null) {
        ...
        if (!TextUtils.isEmpty(textcenter)) {
            /*避免中间的内容超出范围*/
//            FLog.e(TAG, "左边占用：" + leftUsed + "   右边占用：" + rightUsed);
...
//            FLog.e(TAG, "   剩余：" + centerTextMaxWidth);
            tv_center_text.measure(0, 0);
//            FLog.e(TAG, "中间标题长度：" + tv_center_text.getMeasuredWidth());
...
//        view_tran_bar.setBackgroundColor(Color);
    ...
    public TitleLayout setTextIconSpace(int textIconSpace) {
        this.textIconSpace = FConversUtils.dip2px(getContext(), textIconSpace);
        return this;
    }

    public TitleLayout setLeftSpace(int leftSpace) {
        this.leftSpace = FConversUtils.dip2px(getContext(), leftSpace);
        return this;
    }

    public TitleLayout setRightSpace(int rightSpace) {
        this.rightSpace = FConversUtils.dip2px(getContext(), rightSpace);
        return this;
    }

    public TitleLayout setCenterRowSpace(int centerRowSpace) {
        this.centerRowSpace = FConversUtils.dip2px(getContext(), centerRowSpace);
        return this;
    }
